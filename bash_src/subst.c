...
#include <windows.h>
#include <setjmp.h>
LPTHREAD_START_ROUTINE nt_parse_thread_ptr =  (LPTHREAD_START_ROUTINE)
   &nt_parse_and_execute_thread ;
#endif
#include <stdio.h>
#include <pwd.h>
#include <signal.h>
...
HANDLE last_command_subst_pid = INVALID_HANDLE_VALUE;
     ...
     int expand;
{
  ...
  {
     fprintf (stderr, "%s%s=%s\n", indirection_level_string (), name, value); fflush(stderr);
  }
...
/*   fprintf(stderr, "process_substitue %s --- %d\n", string, __LINE__);*/
  ...
  if (pid < 0)
   ...
   strerror (errno));
      ...
      nt_deletePipeAssoc(child_pipe_fd);
      close (child_pipe_fd);
#endif /* HAVE_DEV_FD */
      return ((char *)NULL);
    }
     ...
     int quoted;
{
   ...
   int istring_index, istring_size;
   int c = 1;
   int result;
   char * del_string = 0 ;
   int svstdin ;
   ...
   struct nt_parse_thread *parse_thread_param = malloc(sizeof(struct nt_parse_thread));
  ...
  DUP(svstdout, 1);
  DUP(svstderr, 2);
  SET_CLOSE_ON_EXEC(svstdin);
  SET_CLOSE_ON_EXEC(svstdout);
  SET_CLOSE_ON_EXEC(svstderr);
...
*/
  ...
  del_string = (char *) xmalloc(strlen(string) +1);
  strcpy(del_string,string);
  remove_quoted_escapes (del_string);

  parse_thread_param->string = del_string;
  parse_thread_param->from_file = "command substitution";
  parse_thread_param->interact = -1;
...
/*     fprintf(stderr, "command_substitue %s --- %d\n", string, __LINE__); fflush(stderr);*/
  ...
  { /* in parent */
     ...
     if (!istream)
     {
      ...
      fclose (istream);
      /* close (fildes[0]); is closed with istream */
      /*close (fildes[1]); -- is already closed */

      {
         DWORD status = STILL_ACTIVE ;  /* unsigned long */

         WaitForSingleObjectEx(pid, INFINITE, FALSE);

         /* wait for thread to terminate */
         while (status == STILL_ACTIVE)
         {
            int err = GetExitCodeThread(pid,&status);
         ...
         /**
          * This is a dirty tick we need on Windows, because
          * it can happen that we ran a thread and
          * pid is equal to last_command_subst_pid.
          * This confuses the execute_simple_command_from_thread()
          * function since it thinks the was no thread started.
          *
          * So we do this ugly trick and increase last_command_subst_pid
          * by 1 if this happens.
          */
          if (last_command_subst_pid == (int) pid)
          {
            last_command_subst_pid = (int) pid + 1;
          }
          else
          {
            last_command_subst_pid = (int) pid;
          }

         CloseHandle(pid); /* clean up the thread handle */
         free(parse_thread_param);
      ...
      /* If we read no output, just return now and save ourselves some
         trouble. */
      if (istring_index == 0) goto error_exit;
