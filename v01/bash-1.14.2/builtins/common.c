...
parse_and_execute (string, from_file, interact)
     ...
     int interact;
{
   return(parse_and_execute_from_thread(string, from_file, interact, 0));
}

int
parse_and_execute_from_thread (string, from_file, interact, from_thread)
     char *string;
     char *from_file;
     int interact;
     int from_thread;
  ...
  with_input_from_string (string, from_file);
  {
	...
	if (parse_command () == 0)
   {
      if ((command = global_command) != (COMMAND *)NULL)
      {
         ...
         last_result = execute_command_internal_from_thread(command, 0, NO_PIPE, NO_PIPE, bitmap, from_thread);
