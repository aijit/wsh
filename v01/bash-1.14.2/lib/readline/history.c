     ...
     int from, to;
{
  ...
  file = OPEN3 (input, O_RDONLY, 0666);
...
#ifdef __NT_VC__
	nt_remove_cr(buffer,finfo.st_size);
#endif
     ...
     register int lines;
{
  ...
  file = OPEN3 (filename, O_RDONLY, 0666);
  ...
  /* Write only if there are more lines in the file than we want to
     truncate to. */
  if (i && ((file = OPEN3 (filename, O_WRONLY|O_TRUNC, 0666)) != -1))
    {
     ...
     int nelements, overwrite;
{
  ...
  if ((file = OPEN3 (output, mode, 0666)) == -1)
    {
      if (output)
	free (output);
     ...
     Suggested by Peter Ho (peter@robosts.oxford.ac.uk). */
  {
    ...
    for (i = history_length - nelements; i < history_length; i++)
      if (line = the_history[i]->line)  buffer_size += 1 + strlen (line);
      else buffer_size++ ;
