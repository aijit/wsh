...
void set_lines_and_columns(int lines, int cols);
...
readline_internal ()
{
  ...
  while (!rl_done)
  {
     ...
     SETJMP(code, readline_top_level);
...
rl_gather_tyi ()
{
  ...
  register int tem = 0, result = -1;
  int chars_avail = 0;
  char input = '\0';

#if defined (FIONREAD) && !defined (__NT_VC__)
...
readline_initialize_everything ()
{
  /* Find out if we are running in Emacs. */
   running_in_emacs = (getenv ("EMACS") != (const char *)NULL);
...
#if defined (TIOCGWINSZ) && !defined(__NT_VC__)
...
#if !defined (__GO32__) && !defined(__NT_VC__)
  if (term_backspace)
    for (i = 0; i < count; i++)
...
crlf ()
{
#if defined (NEW_TTY_DRIVER) && ! defined(__NT_VC__)
...
rl_tty_status (count, key)
     int count, key;
{
#if defined (TIOCSTAT) && !defined __NT_VC__
