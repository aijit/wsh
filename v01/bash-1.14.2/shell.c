...
int yyparse();
reset_parser ();
void initialize_filename_hashing ();
   ...
   char **argv, **env;
{
   ...
   nt_init_jmp_critsec();
	...
	nt_main_thread_id = nt_get_thread_id();

   if (0) if (isatty (fileno (stdin))) {
		fclose (stdin);
		i = OPEN("CONIN$",_O_RDONLY); /* make console shared input */
		dup2(i,0);
		CLOSE(i);
	}
#endif
   ...
   posixly_correct = (getenv ("POSIXLY_CORRECT") != (const char *)NULL) ||
      (getenv ("POSIX_PEDANTIC") != (const char *)NULL);
...

   {
      ...
      SETJMP(jmp_ret, subshell_top_level);
      if (jmp_ret)
      {
         argc = subshell_argc;
         argv = subshell_argv;
   ...
   if (shell_initialized || shell_name)
   {
      ...
      shell_reinitialize ();
      {
         /* if (SETJMP(top_level))  exit (2);*/

         int jmp_ret = 0;
         SETJMP(jmp_ret, top_level);
         if (jmp_ret) exit(2);
      }
   }
   ...
   while ((arg_index != argc) && (NULL != argv[arg_index]) && *(argv[arg_index]) == '-')
   {
...
#ifdef __NT_VC__
	nt_remove_cr(string, file_info.st_size);
#endif
  ...
  while (!EOF_Reached)
  {
     ...
     if (read_command () == 0)
     {
        if (global_command)
        {
          ...
          exec_done:
#ifdef __NT_VC__
           nt_set_unlink_name(0); /* delete any temporary file */
           nt_cleanup_childs_table(); /* clean up the childs table */
           nt_cleanup_exec_stdhandles(); /* clean the std handles table */
#endif
           if (current_command)
	        {
              dispose_command (current_command);
...
#ifndef __NT_VC__ /* no long jumps in an interrupt */
