  ...
  while (string = env[string_index++])
      ...
      else
   {
     ...
     if (4 == strlen(name))
     {
        ...
        for (iIndex = 0; iIndex < sizeof(acName); iIndex ++)
        {
           acName[iIndex] = toupper(name[iIndex]);
        }
        if (0 == strcmp(acName, "PATH"))
        {
           char *pcString = malloc(strlen(string)+1);

           if (NULL == pcString)
           {
              v = bind_variable (acName, string); /* original code */
           }
           else
           {
              int iStrLen = strlen(string);

              strcpy(pcString, string);
              ...
              v = bind_variable (acName, pcString);
              free(pcString);
              pcString = NULL;
           }
        }
        ...
        {
           v = bind_variable (name, string); /* original code */
        }
     }
     else
     {
...
/* export the given list of variables to a file */
void nt_print_var_list(FILE * fp, SHELL_VAR ** list)
{
   ...
   for (i = 0; list && (var = list[i]); i++)
   {
      if ((!function_p(var)) &&
          (!exported_p(var)) &&
          (!readonly_p(var)) && (!invisible_p(var))
          && var->value)
      {
         int i = 0;
         int iMax = strlen(var->value);
...
/* export the given list of functions and their definitions to a file */
void nt_print_func_list(FILE *fp, SHELL_VAR **list)
{
   ...
   for (i=0; list && (var = list[i]); i++)
   {
      if (function_p (var) && var->value)
      {
         fprintf(fp, "\n%s%s\n",
                 var->name,
                 named_function_string ((char *)NULL, function_cell(var), 1));
    }
   }
}
